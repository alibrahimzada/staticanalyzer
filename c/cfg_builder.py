"""CFG builder for C using tree-sitter.

The implementation mirrors the Java builder. When ``tree_sitter`` and the
``tree_sitter_c`` bindings are available a proper parser is used.  If not, the
builder falls back to a very naive line based parser so that tests depending on
basic functionality still run.
"""
...
class CFGBuilder:
    def __init__(self, separate=False):
        ...
        try:
            if Language is not None and Parser is not None and tsc is not None:
