"""CFG builder for Go using tree-sitter.

This builder works like the Java one. If ``tree_sitter`` and the
``tree_sitter_go`` bindings are available a real parser is used; otherwise it
falls back to a very naive line based parser so that basic functionality still
works.
"""
...
class CFGBuilder:
    def __init__(self, separate=False):
        ...
        self.src = ""
        # ``tree_sitter_languages`` or ``tree_sitter_go`` may not be installed
